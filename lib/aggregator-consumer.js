...
async function mergeEnrichmentData(jobs) {
  try {
    ...
    console.log(`   📊 Enrichment merged: ${merged}/${jobs.length} jobs (${(merged/jobs.length*100).toFixed(0)}%)`);
    return jobs;
  } catch (err) {
    console.log(`   ⚠️ Enrichment merge skipped: ${err.message}`);
    return jobs;
  }
}
  ...
  return {
    ...
    async fetchJobsWithDiagnostics() {
      try {
        ...
        // Apply 14-day date filter (aggregator outputs full window; consumer trims to active)
        ...
        if (verbose) {
          console.log(`📅 After 14-day filter: ${recentJobs.length} jobs (removed ${allJobs.length - recentJobs.length} older)`);
        }
        ...
        // Merge enrichment data (skills, visa, summary) from enriched_jobs.json.
        // Uses job ID to join. Silent on failure — jobs returned without enrichment.
        // Note: enrichment merge uses the ORIGINAL job ID before format conversion,
        // but formatConverter preserves the id field. If IDs don't match,
        // merge silently returns 0 matches (safe).
