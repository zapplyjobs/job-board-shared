...
test('KEYWORD-10: Network Engineer → not software', () => {
  const domains = tagDomains(makeJob('Network Engineer'));
  assert.ok(!domains.includes('software'), 'network engineer should not be software');
});

test('KEYWORD-10: IP Network Engineer II → not software', () => {
  const domains = tagDomains(makeJob('IP Network Engineer II'));
  assert.ok(!domains.includes('software'), 'IP network engineer should not be software');
});

test('KEYWORD-10: Network Engineer Campus WLAN → not software', () => {
  const domains = tagDomains(makeJob('Network Engineer – Campus WLAN/LAN'));
  assert.ok(!domains.includes('software'));
});

// systems administrator — removed entirely
test('KEYWORD-10: Systems Administrator → not software', () => {
  const domains = tagDomains(makeJob('Systems Administrator'));
  assert.ok(!domains.includes('software'));
});

test('KEYWORD-10: Jr. Systems Administrator → not software', () => {
  const domains = tagDomains(makeJob('Jr. Systems Administrator'));
  assert.ok(!domains.includes('software'));
});

test('KEYWORD-10: Linux Systems Administrator → not software', () => {
  const domains = tagDomains(makeJob('Linux Systems Administrator'));
  assert.ok(!domains.includes('software'));
});

// system administrator (singular) — removed entirely
test('KEYWORD-10: System Administrator → not software', () => {
  const domains = tagDomains(makeJob('System Administrator'));
  assert.ok(!domains.includes('software'));
});

test('KEYWORD-10: Windows System Administrator → not software', () => {
...
test('KEYWORD-10: IT Service Desk Intern → still software via it service keyword', () => {
  const domains = tagDomains(makeJob('IT Service Desk Intern'));
  // NOTE: matches 'it service' keyword (separate from 'service desk' removal).
  // 9 US-SW FPs via 'it service' — outside KEYWORD-10 scope. Track separately.
  assert.ok(domains.includes('software'), 'matches it service, not service desk');
});
...
// it systems: admin/AV/ops/manager variants removed
test('KEYWORD-11: IT Systems Administrator → not software', () => {
  const domains = tagDomains(makeJob('IT Systems Administrator'));
  assert.ok(!domains.includes('software'), 'sysadmin is IT ops');
});
...
test('KEYWORD-11: IT Systems Administrator Launch → not software', () => {
  ...
  assert.ok(!domains.includes('software'), 'sysadmin at SpaceX');
});
