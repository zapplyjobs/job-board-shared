...
function formatMessage(level, message, context = {}) {
  ...
  // Simplify context for CI environment - only show important keys
  let contextStr = '';
  if (Object.keys(context).length > 0) {
    // In CI (GitHub Actions), show compact context
    if (process.env.CI || process.env.GITHUB_ACTIONS) {
      const importantKeys = ['error', 'count', 'jobs', 'remaining', 'status', 'query'];
      const filtered = {};
      importantKeys.forEach(key => {
        if (context[key] !== undefined) filtered[key] = context[key];
      });
      if (Object.keys(filtered).length > 0) {
        contextStr = ' ' + JSON.stringify(filtered);
      }
    } else {
      // Local development - show all context
...
/**
 * Fatal level logging - exits process with code 1
 */
