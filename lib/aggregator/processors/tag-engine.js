...
const ENTRY_LOCK_RE = /\b(new\s+grad|new\s+graduate|university\s+grad|campus\s+hire|early[\s-]career|entry[\s-]level|junior|trainee|associate\s+(?:engineer|developer|analyst|scientist|researcher|director|architect)|assistant\s+(?:vice\s+president|vp)|sr\.?\s*associate|senior\s+associate|principal\s+associate|executive\s+assistant)\b/i;
...
function tagDomains(job, options) {
  ...
  // TAG-11 C47: 'project engineer' — civil/water/structural project engineers are not software (RE/SPEC, Veolia)
  // S262: extended guard to include mechanical/civil modifiers (Veolia mech commissioning, RE/SPEC civil)
  // Guard fires when 'project engineer' is sole sw match AND title has civil-engineering modifier
  const isCivilProjectEngineer = /\b(structural|water resources|water treatment|geotechnical|wastewater|mechanical|civil)\b/i.test(title);
  ...
  if (matchesHardware) {
    ...
    if (!(isOnlyServiceTech && isCustomerServiceTech) && !isCloudOpsEng && !isWaterUtilityFSR && !isVeoliaFieldOps && !isIntuitiveClinicalFP && !isNCRFieldOps && !isBakerFieldOps && !isTradesOnly && !isSoftwarePrimaryRole && !isSupplyChainEng) {
