...
// O*NET domain lookup — 12,084 title variants (3+ words) mapped to our domains via SOC codes.
// Source: O*NET v29.1 alternate_titles.txt + occupation_data.txt (public domain, CC BY 4.0).
// Indexed by contained words for fast lookup — only checks O*NET titles that share a word
// with the job title, avoiding 12K substring scans per job.
const ONET_LOOKUP = (() => {
  try {
    const data = require(path.join(__dirname, 'onet-domain-lookup.json'));
    const entries = Object.entries(data).sort((a, b) => b[0].length - a[0].length);
    // Build word → [entry indices] index for fast filtering
    const wordIndex = new Map();
    entries.forEach(([title], idx) => {
      for (const word of title.split(/\s+/)) {
        if (word.length < 4) continue; // skip short words (the, and, for, etc.)
    ...
    console.warn('tag-engine: onet-domain-lookup.json not found — O*NET fallback disabled');
    return { entries: [], wordIndex: new Map() };
  }
})();

// O*NET 2-word lookup — uses word-boundary regex to prevent substring collisions.
// "air technician" must NOT match "Repair Technician" (substring "air" inside "repair").
// Pre-compiled regexes indexed by first word for fast filtering.
const ONET_2WORD = (() => {
  try {
    const data = require(path.join(__dirname, 'onet-2word-lookup.json'));
    const entries = [];
    const firstWordIndex = new Map();
    for (const [title, domain] of Object.entries(data)) {
      const words = title.split(/\s+/);
      const re = new RegExp('\\b' + title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b', 'i');
      const idx = entries.length;
      entries.push({ title, domain, re });
      // Index by first word (4+ chars) for fast candidate filtering
      const firstWord = words.find(w => w.length >= 4) || words[0];
      if (!firstWordIndex.has(firstWord)) firstWordIndex.set(firstWord, []);
      firstWordIndex.get(firstWord).push(idx);
    }
    return { entries, firstWordIndex };
  } catch {
    return { entries: [], firstWordIndex: new Map() };
  }
})();
...
function tagDomains(job, options) {
  ...
  if (manufacturingKeywords.some(kw => title.includes(kw))) {
    pushTag('manufacturing', findMatch(manufacturingKeywords, title), 'title');
  }
  ...
  // Description fallback: for jobs with no title-based domain match, check description.
  // Only runs when: (1) no domain matched from title, (2) description is non-empty.
  // Uses stricter matching than title — requires 2+ domain-specific phrases to avoid
  // boilerplate contamination. SOFTWARE-TITLE-ONLY (S229) showed that single-keyword
  // description matching causes 1,774 FPs in software. The 2-match threshold prevents this.
  // This recovers GH/Lever/Ashby US jobs that have descriptions but ambiguous titles.
  // WD/SR/EF have no inline descriptions — unaffected.
  if (tags.length === 0 && description.length > 50) {
    const cleanDesc = description.replace(/<[^>]+>/g, ' ').replace(/&[a-z]+;/g, ' ').toLowerCase();

    // Healthcare — clinical/medical terms (not company mission statements)
    const healthDescKw = ['patient care', 'clinical setting', 'medical record', 'hipaa', 'nursing staff', 'hospital', 'bedside', 'healthcare provider', 'clinical experience', 'licensed nurse', 'patient safety', 'medical terminology'];
    if (healthDescKw.filter(kw => cleanDesc.includes(kw)).length >= 2) {
      pushTag('healthcare', '(description: 2+ clinical terms)', 'description-fallback');
    }

    // Finance — financial regulatory/professional terms
    if (tags.length === 0) {
      const financeDescKw = ['financial statements', 'banking regulation', 'series 7', 'series 63', 'finra', 'investment portfolio', 'securities', 'fiduciary', 'audit report', 'gaap', 'ifrs'];
      if (financeDescKw.filter(kw => cleanDesc.includes(kw)).length >= 2) {
        pushTag('finance', '(description: 2+ financial terms)', 'description-fallback');
      }
    }

    // Legal — legal-specific terms
    if (tags.length === 0) {
      const legalDescKw = ['legal counsel', 'litigation', 'contractual', 'regulatory compliance', 'bar admission', 'juris doctor', 'legal department'];
      if (legalDescKw.filter(kw => cleanDesc.includes(kw)).length >= 2) {
        pushTag('legal', '(description: 2+ legal terms)', 'description-fallback');
      }
    }
  }

  // O*NET taxonomy fallback: for jobs with no keyword or description match,
  // check against 12,084 O*NET alternate job title variants (3+ words, mapped to
  // domains via SOC codes). Uses word index for fast filtering, then longest-match-first.
  // Source: O*NET v29.1 (public domain). See TAG_CLASSIFICATION_RESEARCH_S233.md.
  if (tags.length === 0 && ONET_LOOKUP.entries.length > 0) {
    // Collect candidate O*NET entries that share a word with the job title
    const titleWords = title.split(/\s+/).filter(w => w.length >= 4);
    const candidateIndices = new Set();
    for (const word of titleWords) {
      const indices = ONET_LOOKUP.wordIndex.get(word);
      if (indices) indices.forEach(i => candidateIndices.add(i));
    }
    // Check candidates in longest-first order (entries are pre-sorted by length)
    const candidates = [...candidateIndices].sort((a, b) => a - b); // preserve length order
    for (const idx of candidates) {
      const [onetTitle, domain] = ONET_LOOKUP.entries[idx];
  ...
  // O*NET 2-word fallback: unambiguous 2-word titles with word-boundary regex.
  // Runs after 3+ word lookup. Word boundaries prevent substring collisions
  // ("air technician" won't match "Repair Technician"). TAG-5 Phase 1b.
  if (tags.length === 0 && ONET_2WORD.entries.length > 0) {
    const titleWords = title.split(/\s+/).filter(w => w.length >= 4);
    for (const word of titleWords) {
      const indices = ONET_2WORD.firstWordIndex.get(word);
      if (!indices) continue;
      for (const idx of indices) {
        const entry = ONET_2WORD.entries[idx];
        if (entry.re.test(title)) {
          pushTag(entry.domain, '(onet-2w: ' + entry.title + ')', 'onet-2word-fallback');
          break;
        }
      }
  ...
  if (debug) return { domains: tags, matches };
  return tags;
}
