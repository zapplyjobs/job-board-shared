...
function tagDomains(job) {
  ...
  if (!isSalesRole && !isRetailClerk && softwareKeywords.some(kw => title.includes(kw))) {
    tags.push('software');
  } else if (!isSalesRole && !isRetailClerk && softwareKeywords.some(kw => description.includes(kw))) {
    tags.push('software');
  }
