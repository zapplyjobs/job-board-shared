...
function tagEmployment(job) {
  ...
  const internRegex = /\b(summer|fall|spring|winter)\s+20\d{2}\b/i; // "Summer 2026", "Fall 2026"
  // "Summer 2026 Engineering" = internship. "Graduate Engineer" = ambiguous (could be new-grad).
  // Only treat "Graduate Engineer" as internship if combined with season/year.
  const seasonalGrad = /\b(summer|fall|spring|winter)\s+20\d{2}\s+graduate\b/i;

  if (internPatterns.some(p => title.includes(p)) || internRegex.test(job.title || '') || seasonalGrad.test(job.title || '')) {
...
function tagDomains(job) {
  ...
  const hardwareKeywords = [
    ...
    'physical design', 'plasma', 'reliability engineer', 'equipment engineer',
