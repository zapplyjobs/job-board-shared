...
function tagDomains(job, options) {
  ...
  // Description fallback removed S233 — 0% contribution measured.
  // all_jobs.json descriptions are stripped (DESC-MIGRATE-1). For freshly-fetched
  // GH/Lever/Ashby jobs, keywords + O*NET classify them before description fallback fires.
  // Re-add if descriptions become available in the pool (AGG architecture change).
