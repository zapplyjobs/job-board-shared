...
function tagDomains(job, options) {
  ...
  const isIssComplianceOfficer = (/\binformation systems? security\b/i.test(title) && /\bofficer\b/i.test(title)) || /\bISSO\b/.test(title);
  ...
  if (matchesHardware) {
    ...
    if (!(isOnlyServiceTech && isCustomerServiceTech) && !isCloudOpsEng && !isWaterUtilityFSR && !isVeoliaFieldOps && !isIntuitiveClinicalFP && !isNCRFieldOps && !isBakerFieldOps && !isTradesOnly) {
