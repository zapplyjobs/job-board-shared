...
function tagDomains(job, options) {
  ...
  const healthcareExact = [
    ...
    'mental health',                // 13 gen, 100% spec — mental health counselors/therapists
  ...
  if (tags.length === 0 && description.length > 100) {
    ...
    // TAG-9 Phase 2 (S237): Expanded phrase lists with domain-specific terms.
    // Boilerplate phrases removed: 'audit' (11%), 'seo' (0.6% but matches Lowe's EEO text).
    // Each phrase verified <5% frequency across all descriptions (not boilerplate).
    // 'cad' (10.5%) rejected as boilerplate. 'discharge'/'prescription' rejected (EEO/benefits text).
    // TAG-9 Phase 7 (S237): expanded from manual review of remaining general job descriptions.
    // All new phrases verified <5% frequency across 56,771 descriptions.
    // Rejected: 'health insurance' (9.8%), 'security clearance' (6%), 'territory' (6.6%), 'quota' (4.7%).
    const descDomains = [
      { domain: 'hardware', phrases: ['thermal design', 'circuit design', 'fpga', 'pcb', 'simulation', 'commissioning', 'equipment calibration', 'test systems', 'embedded system', 'signal integrity', 'power supply', 'schematic',
        'solidworks', 'autocad', 'electrical engineering', 'mechanical design', 'failure analysis', 'bill of materials', 'prototyping', 'oscilloscope', 'wiring diagram',
      ...
      { domain: 'software', phrases: ['source code', 'code review', 'ci/cd', 'api design', 'microservice', 'database design', 'incident response', 'deployment pipeline', 'version control', 'agile development', 'sprint planning', 'unit test',
        'kubernetes', 'docker', 'terraform', 'containerization', 'programming language', 'codebase', 'automated testing', 'pull request', 'technical debt',
        'endpoint', 'virtualization', 'firewall', 'encryption', 'load balancer', 'test automation', 'regression test', 'dns'] },
      ...
      { domain: 'finance', phrases: ['financial statements', 'treasury', 'fund administration', 'gaap', 'ifrs', 'portfolio management', 'credit risk', 'financial reporting',
        'balance sheet', 'income statement', 'reconciliation', 'general ledger', 'journal entry', 'month-end close', 'accounts payable', 'accounts receivable', 'internal controls',
        'wealth management', 'investment management', 'fund accounting', 'asset management', 'fixed income', 'capital markets',
      ...
      { domain: 'healthcare', phrases: ['patient care', 'clinical trial', 'hipaa', 'therapeutic area', 'medical device', 'pharma', 'drug safety', 'clinical research', 'fda', 'healthcare provider',
        'vital signs', 'triage', 'infection control', 'sterile', 'specimen', 'blood draw', 'inpatient', 'outpatient', 'nursing assessment', 'informed consent',
        'patient outcomes', 'clinical operations', 'regulatory affairs', 'medical affairs', 'drug development', 'therapeutics'] },
      ...
      { domain: 'retail', phrases: ['store operations', 'merchandising', 'product categories', 'store manager'] },
    ];
