...
function tagJob(job) {
  ...
  taggedJob.tags = {
    ...
    // Internships default to no_experience — description context ("our team has 5+ years")
    // causes false 3_plus_years tags on 86% of intern jobs
    experience: employment === 'internship' ? 'no_experience' : tagExperience(job),
...
function tagDomains(job) {
  ...
  // Nursing domain (title-only — 'rn'/'lpn'/'cna' are short so use word-boundary regex)
  const nursingExact = ['registered nurse', 'nurse practitioner', 'nursing', 'travel nurse', 'lvn', 'graduate nurse'];
...
function tagLocations(job) {
  ...
  if (isExplicitlyNonUS) {
    ...
    const usStates = [
      ...
      'united states', '- usa', ', usa', '(usa)', 'u.s.a'
    ];
...
function tagExperience(job) {
  ...
  // Default unknown — most entry-level jobs don't state years explicitly
  return 'unknown';
}
