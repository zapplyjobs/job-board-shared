...
function tagEmployment(job) {
  ...
  if (internWordRegex.test(job.title || '') || seasonalRegex.test(job.title || '') || seasonalGrad.test(job.title || '')) {
    ...
    const fakePatterns = ['senior intern', 'sr. intern', 'principal intern'];
...
function tagDomains(job, options) {
  ...
  if (deptRaw) {
    const DEPT_RULES = [
      ...
      // S262 domain-accuracy audit: removed 'people' and 'talent' as standalone triggers.
      ...
      // S262 domain-accuracy audit: removed bare 'design' as standalone trigger.
  ...
  const softwareKeywords = [
    ...
    // TAG-11 S239 C37: Bosch software
    'it technician',                // 1 gen, 0 FP — IT Technician 1st shift (Bosch)
    // TAG-11 S241 C38: AbbVie software/IT roles
    ...
    // TAG-11 S242 C39: CACI (ATS: Information Technology)
    ...
    // TAG-11 S243 C40: Northrop Grumman IT/cyber
    ...
    // TAG-11 S243 C40: Guidehouse technology consulting
    ...
    // TAG-11 S248 C42: Guidehouse health/software technology consulting
    ...
    // TAG-11 S248 C43: General Motors UX/digital
    ...
    // TAG-11 S245 C41: Caterpillar IT/digital roles
    ...
    // TAG-11 S250 C44: Abbott software roles
    'decision support',             // 1 gen, 0 FP — Decision Support Tools Engineer (Abbott)
    // TAG-11 S252 C45: Warner Bros. Discovery IT/broadcast/game engineering
    ...
    // TAG-11 S255 C48: CACI (ATS: Information Technology / Intelligence)
    'navwar assessment',            // 1 gen, 0 FP — NAVWAR Assessment Coordinator (CACI; Naval Warfare Systems Center IT assessment)
    // TAG-11 S255 C48: Vanguard (Global Risk and Security)
    ...
    // TAG-11 S256 C50: Northrop Grumman + RTX (inverted title formats)
    ...
    // TAG-11 S257 C52: Capital One software roles
  ...
  // S262: civil/water/mechanical modifiers (Veolia, RE/SPEC).
  ...
  if (!isSalesRole && !isRetailClerk && !isIssComplianceOfficer && matchesSoftware(title) && !isGuardedSwOnly(title)) {
  ...
  const aiKeywords = [
    ...
    // TAG-11 S243 C40: Guidehouse language AI
  ...
  const dataScienceKeywords = [
    ...
    // TAG-11 S241 C38: AbbVie analytics roles
  ...
  const hardwareKeywords = [
    ...
    // S262 domain-accuracy audit: 'manufacturing engineer' moved to manufacturingKeywords.
    ...
    // TAG-11 S239 C37: Bosch hardware
    ...
    // TAG-11 S242 C39: Leidos defense hardware
    ...
    // TAG-11 S242 C39: Leidos defense hardware (via missile systems)
    'missile launcher',                   // 1 gen, 0 FP — Missile Launcher Product Team Lead (Leidos)
    // TAG-11 S242 C39: SpaceX loads engineering
    'loads & dynamics',                   // 1 gen, 0 FP — Loads & Dynamics Engineer Starship (SpaceX)
    // TAG-11 S243 C40: Northrop Grumman defense hardware
    ...
    // TAG-11 S245 C41: Caterpillar hardware/engineering roles
    ...
    // TAG-11 S248 C43: General Motors vehicle/powertrain hardware
    ...
    // TAG-11 S256 C49: Intuitive Surgical (vision/imaging hardware)
    'vision imaging',            // 1 gen, 0 FP — Vision Imaging Engineer (Intuitive; imaging algorithms, camera/optics for da Vinci robot)
    // TAG-11 S256 C50: Northrop Grumman + RTX (defense hardware, inverted title formats)
    ...
    // TAG-11 S258 C53: Boeing hardware roles
  ...
  if (matchesHardware) {
    ...
    // TAG-11 S253 C46: 'field service representative' — water treatment/utility FSRs are
    ...
    // must NOT be guarded globally. Baker Hughes guarded separately (S268A — oilfield, not tech).
    ...
    // S268A: NCR Voyix field service technicians repair ATMs/POS terminals — operations,
    ...
    // S268A: Baker Hughes field service/maintenance roles are oilfield industrial — not tech
    ...
    // S268A: Trades keyword guard — hvac, electrician, maintenance technician, journeyman,
    ...
    const isTechnicianOnly = (() => {
      ...
      return !hasEngContext;
    })();
    if (!isOnlyServiceTech && !isCloudOpsEng && !isWaterUtilityFSR && !isVeoliaFieldOps && !isIntuitiveClinicalFP && !isNCRFieldOps && !isBakerFieldOps && !isTradesOnly && !isSoftwarePrimaryRole && !isSupplyChainEng && !isTechnicianOnly) {
  ...
  const healthcareExact = [
    ...
    // TAG-11 S241 C38: AbbVie pharma R&D/science roles
    ...
    // TAG-11 S250 C44: Abbott healthcare roles
  ...
  const financeKeywords = [
    ...
    // TAG-11 S257 C52: Capital One finance roles
  ...
  const marketingKeywords = [
    ...
    // TAG-11 S241 C38: AbbVie pharma marketing
    ...
    // TAG-11 S256 C49: Applied Materials (market research intern)
    ...
    // TAG-11 S257 C51: NBCUniversal marketing roles
    ...
    // TAG-11 S257 C52: Capital One marketing roles
  ...
  const operationsKeywords = [
    ...
    // TAG-6 S233: promoted from O*NET
    // S262: 'fulfillment associate' moved to logisticsKeywords — warehouse/fulfillment is logistics not operations.
    ...
    // TAG-9 S237: Phase 3 keyword expansion
    // S262: 'fulfillment team lead' moved to logisticsKeywords (same reason as fulfillment associate)
    ...
    // TAG-10 S238: title keyword gap analysis
    // S262: 'fulfillment associate' and 'fulfillment team' moved to logisticsKeywords
    // TAG-11 S238: adaptive classifier cycle 1
    ...
    // TAG-11 S241 C38: AbbVie operations/program management
    ...
    // TAG-11 S239 C37: Bosch operations
    ...
    // TAG-11 S242 C39: CACI intelligence/operations
    ...
    // TAG-11 S242 C39: CACI linguist/interpreter roles
    ...
    // TAG-11 S242 C39: Booz Allen Hamilton defense/government operations
    ...
    // TAG-11 S242 C39: Leidos defense operations
    ...
    // TAG-11 S242 C39: SpaceX operations
    ...
    // TAG-11 S243 C40: Northrop Grumman operations
    ...
    // TAG-11 S245 C41: Caterpillar operations roles
    ...
    // TAG-11 S256 C49: Intuitive Surgical (operations/continuous improvement)
    ...
    // TAG-11 S256 C50: Northrop Grumman + RTX (defense ops)
    ...
    // TAG-11 S257 C51: NBCUniversal operations roles (theme park, broadcast, media production)
    ...
    // TAG-11 S257 C52: Capital One operations roles (banking/finance ops)
    ...
    // TAG-11 S258 C53: Boeing operations roles
  ...
  const hrKeywords = [
    ...
    // S262 domain-accuracy audit: removed 'executive assistant' and 'administrative assistant'.
    ...
    // TAG-11 S242 C39: SpaceX
    'benefits coordinator',         // 1 gen, 0 FP — Benefits Coordinator (SpaceX)
    // TAG-11 S256 C49: Intuitive + Medtronic + Snap (employee relations)
    ...
    // TAG-11 S257 C51: NBCUniversal + Proofpoint HR
    ...
    // TAG-11 S257 C52: Capital One HR roles
  ...
  // S262 domain-accuracy audit: 'manufacturing engineer' moved HERE from hardwareKeywords.
  ...
  const manufacturingKeywords = [
    'manufacturing engineer', 'manufacturing engineering', // S262: moved from hardware — 132 misroutes fixed
    ...
    // TAG-11 S241 C38: AbbVie manufacturing/lab operations
    ...
    // TAG-11 S239 C37: Bosch manufacturing
    ...
    // TAG-11 S242 C39: SpaceX manufacturing/coatings
    'coatings technician',           // 1 gen, 0 FP — Paint & Coatings Technician (SpaceX)
    // TAG-11 S245 C41: Caterpillar manufacturing/production roles
    ...
    // TAG-11 S256 C50: Northrop Grumman + RTX + Oshkosh (defense/industrial manufacturing)
  ...
  const logisticsKeywords = [
    'fulfillment associate', // S262: moved from operationsKeywords — 225 Lowe's/warehouse jobs now correctly logistics
    'fulfillment team lead', // S262: moved from operationsKeywords — same reason
    ...
    // TAG-11 S256 C50: Northrop Grumman (SCA = Service Contract Act govt contractor roles)
  ...
  // S262: Pre-O*NET executive/admin assistant guard.
  ...
  if (tags.length === 0) {
    ...
    // "systems administrator", and "service desk" to software. These are not SWE roles.
    const isOnetSwFP = (
      /\b(network engineer|systems? administrator|service desk)\b/i.test(title) &&
...
function tagLocations(job) {
  ...
  // AGG-LOC-3: If job_state was extracted and it's a 2-letter code that's NOT a US state
  // abbreviation, the job is almost certainly non-US (e.g., 'UK', 'ON', 'BC').
  // validator.js extracts state codes from location strings like "Bristol, UK" → job_state='UK'.
  const US_STATE_ABBRS_EARLY = new Set([
    'al','ak','az','ar','ca','co','ct','de','fl','ga','hi','id','il','in','ia',
    'ks','ky','la','me','md','ma','mi','mn','ms','mo','mt','ne','nv','nh','nj',
    'nm','ny','nc','nd','oh','ok','or','pa','ri','sc','sd','tn','tx','ut','vt',
    'va','wa','wv','wi','wy','dc'
  ]);
  if (job.job_state && job.job_state.length === 2 &&
      !US_STATE_ABBRS_EARLY.has(job.job_state.toLowerCase())) {
    return tags;
  }
  ...
  if (job.is_us_only === true) {
    ...
    if (hasUsKeyword) {
      ...
      if (hasNonUS) {
        ...
        const hasRealNonUSCountry = hasMexico || /\bunited kingdom\b|\buk\b|\baustralia\b|\bgermany\b|\bfrance\b|\bjapan\b|\bswitzerland\b|\bsweden\b|\bnetherlands\b|\bireland\b|\bsingapore\b|\bbrazil\b|\bcanada\b|\bcolombia\b|\bspain\b|\bisrael\b|\bsouth korea\b|\bnew zealand\b|\bsouth africa\b|\bpoland\b|\bczech\b|\bdenmark\b|\bnorway\b|\bfinland\b|\bbelgium\b|\baustria\b|\bitaly\b|\bportugal\b|\bhungary\b|\bromania\b|\bturkey\b|\btaiwan\b|\bindia\b|\bchina\b|\bargentina\b|\bchile\b|\bperu\b|\bcosta rica\b|\bsaudi arabia\b|\bukraine\b|\begypt\b|\bnigeria\b|\bpakistan\b|\bbangladesh\b|\bvietnam\b|\bindonesia\b|\bthailand\b|\bmalaysia\b|\bphilippines\b|\bgreece\b/i.test(locationStr);
