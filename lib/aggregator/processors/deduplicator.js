...
const INTERNSHIP_TTL_DAYS = 120; // Internships are posted months before start date — SUP-TTL-1
...
function cleanupOldEntries(store) {
  const now = Date.now();
  const cutoff = now - INTERNSHIP_TTL_MS; // Use wider window — store has no tag data (SUP-TTL-1)
...
function deduplicateJobs(jobs) {
  ...
  for (const job of jobs) {
    ...
    if (seenById || seenByFp) {
      ...
      // Internships get a wider window (SUP-TTL-1) — posted months before start date.
      const jobTtlMs = job.tags?.employment === 'internship' ? INTERNSHIP_TTL_MS : DEDUPE_TTL_MS;
      const jobCutoff = now - jobTtlMs;
      if (ts < jobCutoff) {
        expiredByDate++;
        continue;
      }
