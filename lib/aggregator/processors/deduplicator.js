...
const DEDUPE_TTL_DAYS = 7; // Remove entries after 7 days (matches rolling window cutoff)
 ...
 *   1. OUTPUT: all jobs within the 7-day active window (full catalog for consumer repos)
...
function deduplicateJobs(jobs) {
  console.log(`🔍 Processing ${jobs.length} jobs for 7-day active window...`);

  // Load existing dedupe store
  const store = loadDedupeStore();

  // Clean up old entries first (TTL-based) — removes entries > 7 days old
  const cleanupStats = cleanupOldEntries(store);

  const activeWindow = []; // All jobs within 7-day window (output)
  ...
  // AGG-DEAD-TENANT-TRACE (Session A9): diagnostic logging for 17 dead tenants.
  // These companies are fetched but their jobs never appear in all_jobs.
  // Logging traces them through the dedup pipeline to identify where they vanish.
  const _deadTenantTracer = new Set([
    'vultr', 'lightspark', 'quora', 'witness ai', 'cognitiv', 'inflection ai',
    'yugabytedb', 'deep genomics', 'immuta', 'leantaas', 'qrypt', 'demiurge studios',
    'teletracking technologies', 'raytheon technologies', 'samsung research america',
  ]);
  let _tracedCount = 0;
  ...
  for (const job of jobs) {
    ...
    // AGG-DEAD-TENANT-TRACE: log dead tenant jobs entering dedup
    const _isTraced = _deadTenantTracer.has((job.company_name || '').toLowerCase());
    if (_isTraced) {
      _tracedCount++;
      const rawPA = job.posted_at ? new Date(job.posted_at).getTime() : 'null';
      console.log(`🔍 TRACE [${job.company_name}] id=${job.id} posted_at=${job.posted_at} rawMs=${rawPA} cutoff=${freshnessCutoff}`);
    }
    ...
    if (seenById || seenByFp) {
      ...
      if (_isTraced) {
        console.log(`🔍 TRACE [${job.company_name}] SEEN: byId=${seenById} byFp=${seenByFp} title="${(job.title||'').substring(0,40)}"`);
      }
      ...
      if (ts < freshnessCutoff) {
        expiredByDate++;
        // AGG-DEAD-TENANT-TRACE: log why this job expired
        if (_isTraced) {
          const ageDays = ((now - ts) / (24*60*60*1000)).toFixed(1);
          console.log(`🔍 TRACE [${job.company_name}] EXPIRED: ts=${ts} cutoff=${freshnessCutoff} age=${ageDays}d title="${(job.title||'').substring(0,40)}"`);
        }
        continue;
      }
      // AGG-DEAD-TENANT-TRACE: log net-new entry
      if (_isTraced) {
        console.log(`🔍 TRACE [${job.company_name}] NET-NEW: entered pool. ts=${ts} title="${(job.title||'').substring(0,40)}"`);
      }
  ...
  // AGG-DEAD-TENANT-TRACE summary
  if (_tracedCount > 0) {
    console.log(`🔍 TRACE SUMMARY: ${_tracedCount} dead-tenant jobs entered dedup, expired=${expiredByDate}`);
  }
