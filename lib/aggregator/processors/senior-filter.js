...
//     NOTE: "Level N" patterns were tested and reverted — too company-specific.
//     Snap uses Level 3/4 for new-grad roles; Northrop uses Level 3 for mid-senior.
//     No safe threshold exists that works across companies.
...
const ENTRY_LEVEL_GUARDS = [
    ...
    /\bshift\s+(lead|manager)\b/i,         // hourly production roles
    /\bmanager\s+trainee\b/i,              // management training program
    /\bcase\s+manager\b/i,                 // healthcare/social work entry role
...
function hasSeniorTitle(title) {
    ...
    // Roman numeral suffix (II/III/IV) and high job-level suffix (L5+)
    if (!matchedSenior) {
        if (SENIOR_ROMAN_PATTERN.test(title)) matchedSenior = true;
        else if (SENIOR_LLEVEL_PATTERN.test(title)) matchedSenior = true;
    }
