...
// Numeric level patterns that indicate mid/senior regardless of keyword list.
// P1: "Level 3" / "Level 5" etc — Northrop Grumman and similar WD tenants use this convention.
//     Level 1/2 = entry, Level 3+ = mid/senior. "Level 2/3" has no space before digit so won't match.
const SENIOR_LEVEL_PATTERN = /\bLevel\s+[3-9]\b/i;

// P2: Roman numeral suffix " II" / " III" / " IV" — Applied Materials, AbbVie, Bosch etc.
//     " I" is intentionally excluded (Associate I, Analyst I are entry-level).
//     Matches at word boundary to avoid partial matches inside longer words.
const SENIOR_ROMAN_PATTERN = /\s+(II|III|IV)\b/i;

// P3: Job-level suffix "L3"–"L9" as a standalone token (DeepMind L5, Snap L5).
...
const SENIOR_LLEVEL_PATTERN = /(?:[^/\w])L[3-9](?:[^/\w]|$)/;
...
function hasSeniorTitle(title) {
    ...
    // Check numeric/roman level patterns
    if (SENIOR_LEVEL_PATTERN.test(title)) return true;
    if (SENIOR_ROMAN_PATTERN.test(title)) return true;
    if (SENIOR_LLEVEL_PATTERN.test(title)) return true;
