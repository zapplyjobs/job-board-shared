...
function parseWorkdayLocation(locationsText, applyUrl) {
    ...
    // Fall back to URL slug to detect US location (WD-F6/F7).
    if (/^\d+ locations?$/i.test(raw)) {
        const isUs = applyUrl ? extractUSFromWorkdaySlug(applyUrl) : false;
        return { job_city: '', job_state: '', location: raw, ...(isUs && { is_us_only: true }) };
    }
 ...
 *
 * @param {Object} tenant - { name, url, site, us_only? } — e.g. { name: "Salesforce", url: "https://salesforce.wd12.myworkdayjobs.com", site: "External_Career_Site" }
    ...
    for (let i = 0; i < tenants.length; i += concurrency) {
        ...
        const results = await Promise.all(batch.map(async tenant => {
            try {
                ...
                if (jobs.length > 0) console.log(`   ✅ ${tenant.name}: ${jobs.length} jobs`);
                else console.log(`   ○ ${tenant.name}: 0 jobs`);
                return jobs;
            } catch (err) {
