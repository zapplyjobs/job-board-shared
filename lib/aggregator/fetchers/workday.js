...
function parseWorkdayLocation(locationsText, applyUrl) {
    ...
    // S268A: "ST-CITY[-CODE]" format — RTX/Raytheon WD internal codes.
    // Examples: "MD-FULTON-8170", "MA-WOBURN-WB1", "AZ-TUCSON-848", "TX-REMOTE",
    //           "TX-HOUSTON-575 N. Dairy Ashford Rd" (with address suffix)
    // First segment: 2-letter state abbr. Second: city name. Rest: site code or address (discard).
    // Match `ST-CITY` then discard site code (dash followed by alphanumeric code at end)
    // or address (space followed by number) or end of string.
    const stCityMatch = raw.match(/^([A-Z]{2})-([A-Za-z][A-Za-z .'-]*?)(?:-[A-Z0-9]+(?:\s.*)?|\s+\d.*|\s*$)/);
...
async function fetchAllWorkdayJobs(tenants, options = {}) {
    const { concurrency = 15, delayMs = 200 } = options;
    const allJobs = [];

    console.log(`\n🔷 Fetching from ${tenants.length} Workday tenants (concurrency: ${concurrency})...`);
    ...
    for (let i = 0; i < tenants.length; i += concurrency) {
        ...
        const results = await Promise.all(batch.map(async tenant => {
            ...
            try {
                ...
                if (jobs.length > 0) console.log(`   ✅ ${tenant.name}: ${jobs.length} jobs (${ms}ms)`);
        ...
        if (delayMs > 0 && i + concurrency < tenants.length) {
