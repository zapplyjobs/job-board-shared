...
function parseWorkdayLocation(locationsText, applyUrl) {
    ...
    // "FacilityName | StreetAddress | City | ST" — Saint Luke's pipe-delimited address format
    // All 4 segments required; city is index 2, state is index 3.
...
async function fetchWorkdayJobs(tenant) {
    ...
    const jobs = allPostings.map(p => normalizeWorkdayJob(p, url, name));
