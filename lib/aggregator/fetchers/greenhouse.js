...
function parseGreenhouseLocation(locationStr) {
    ...
    if (!primary || /^(us|usa|remote|remote-friendly|all locations|select locations)/i.test(primary)) {
    ...
    const cityStateMatch = parts[0] && parts[1] && /^[A-Z]{2}$/.test(parts[1]);
    if (cityStateMatch) {
...
async function fetchAllGreenhouseJobs(companies, options = {}) {
    ...
    console.log(`\n🌿 Fetching from ${companies.length} Greenhouse boards (concurrency: ${concurrency})...`);
