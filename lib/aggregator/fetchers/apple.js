 ...
 * @param {number} [options.previousTotalRecords] - Total records from prior run (for incremental skip).
...
async function fetchAllAppleJobs({ previousJobCount = 0, previousJobIds, previousTotalRecords } = {}) {
  ...
  while (page <= pageLimit) {
    ...
    if (page === 1) {
      ...
      if (!needsFullFetch && previousJobIds && previousJobIds.size > 0 && previousTotalRecords) {
        const page1Ids = extracted.jobs.map(j => `apple-${j.positionId || j.reqId}`);
        const allKnown = page1Ids.every(id => previousJobIds.has(id));
        const sameTotal = totalRecords === previousTotalRecords;

        if (allKnown && sameTotal) {
          console.log(`  ⚡ Incremental skip: page-1 IDs unchanged + totalRecords match (${totalRecords}). Skipping ${Math.min(totalPages, pageLimit) - 1} pages.`);
          for (const job of extracted.jobs) {
            const normalized = normalizeAppleJob(job);
          ...
          const reason = !allKnown ? 'new jobs on page 1' : 'totalRecords changed';
          console.log(`  📄 Full fetch needed: ${reason}. Proceeding with ${pageLimit} pages.`);
        }
      }
