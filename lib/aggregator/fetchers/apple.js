 ...
 */
const FIRST_RUN_THRESHOLD = 2000;
...
async function fetchAllAppleJobs({ previousJobCount = 0 } = {}) {
  ...
  if (needsFullFetch) {
    console.log(`  🔄 Full fetch mode: previous count ${previousJobCount} < ${FIRST_RUN_THRESHOLD} threshold`);
  }
  ...
  console.log(`  Apple total: ${allJobs.length} jobs (${page} pages)${needsFullFetch ? ' (full fetch)' : ' (routine cap)'}`);
  return allJobs;
}
