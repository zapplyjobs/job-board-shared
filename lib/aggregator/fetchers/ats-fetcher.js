 ...
 * - Workday (52 tenants)
 * - Eightfold (3 tenants)
 * - SmartRecruiters (17 companies)
 *
 * Total: 284 companies/tenants
 */
...
async function fetchFromAllATS(options = {}) {
    ...
    try {
        ...
        if (sources.includes('smartrecruiters') && companyList.smartrecruiters && companyList.smartrecruiters.length > 0) {
            const srJobs = await fetchAllSmartRecruitersJobs(companyList.smartrecruiters, { delayMs });
            results.jobs.push(...srJobs);
            results.stats.by_source.smartrecruiters = srJobs.length;
        }
