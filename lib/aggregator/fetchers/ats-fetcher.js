...
async function fetchFromAllATS(options = {}) {
    ...
    try {
        ...
        // AGG-22: Eightfold gated — 2 tenants (BCG, Zebra), 55 jobs, 0 tech-US, 0 enrichment.
        // Pagination broken (API caps 10/page). Re-enable when AGG-19 is fixed or tenants produce tech-US.
        const EF_ENABLED = false;

        if (EF_ENABLED && sources.includes('eightfold') && companyList.eightfold && companyList.eightfold.length > 0)
