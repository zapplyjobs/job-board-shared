...
async function fetchWorkdayJobs(tenant) {
    ...
    // Normalize: ensure URL has scheme ( tolerate entries missing https:// )
    const url = rawUrl.startsWith('http') ? rawUrl : `https://${rawUrl}`;
    let hostname;
    try {
        hostname = new URL(url).hostname;
    } catch (e) {
        console.log(`   ⚠️ Workday: invalid URL for ${name}: ${rawUrl} — skipping`);
        return { jobs: [], total: 0 };
    }
...
async function probeWorkdayTotal(tenant) {
    const url = tenant.url.startsWith('http') ? tenant.url : `https://${tenant.url}`;
    let hostname;
    try {
        hostname = new URL(url).hostname;
    } catch (e) {
    ...
    const endpoint = `${url}/wday/cxs/${tenantSlug}/${tenant.site}/jobs`;
