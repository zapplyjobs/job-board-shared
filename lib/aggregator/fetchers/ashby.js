...
function normalizeAshbyJob(job, companySlug, companyName) {
    ...
    return {
        ...
        // If the original date is older than 14 days, use fetch time instead — same logic as Lever fix.
        posted_at: (() => {
...
async function fetchAllAshbyJobs(companies, options = {}) {
    ...
    console.log(`\n🔷 Fetching from ${companies.length} Ashby boards (concurrency: ${concurrency})...`);
