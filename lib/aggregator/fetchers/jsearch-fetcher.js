 ...
 * - 16 queries total (software:4, datascience:4, hardware:4, nursing:4)
...
// Publishers to exclude for data science queries — JS-2 audit found 50% noise in data analyst query
const DS_EXCLUDE_PUBLISHERS = [
  'VirtualVocations',
  'SynergisticIT',
];
...
async function fetchFromJSearch() {
  ...
  try {
    ...
    const nursingExtraParams = {
      ...
      exclude_job_publishers: NURSING_EXCLUDE_PUBLISHERS.join(','),
    };

    const dsQuerySet = new Set(QUERY_SETS.datascience);
    const dsExtraParams = {
      exclude_job_publishers: DS_EXCLUDE_PUBLISHERS.join(','),
    };
    ...
    for (let i = 0; i < queries.length; i++) {
      ...
      const extraParams = isNursing ? nursingExtraParams : isDS ? dsExtraParams : {};
