 ...
 * Paid tier: 10,000 requests/month (~333/day). Cap: 320/day (safety margin).
 *
 * Features:
 * - Per-run quota: floor(320/96) = 3 queries per run (288/day, 90% of 320 cap).
 * - Hard cap: DAILY_REQUEST_CAP = 320 — never exceeds even if run frequency changes.
 ...
 * UPDATED 2026-03-11 (JS-5): Removed 4 nursing queries (covered by Workday/GH); added 4 tech queries.
 *   Swapped firmware → RF engineer (firmware supply collapsed to 2/run).
 * UPDATED 2026-03-11 (INTERN-2a): Replaced 6 entry-level-only queries with intern-specific queries.
 *   Goal: increase intern signal. 93% of JSearch interns are additive (not in ATS pool).
 *   Replaced: 'new grad sw eng', 'entry level data scientist', 'ml engineer entry level',
 *             'entry level hw eng', 'entry level AI engineer', 'systems engineer entry level',
 *             'associate software developer'.
 *   Quota: unchanged — 3 queries per run × 96 runs/day = 288 requests/day (90% of 320 cap).
 * - 24 queries total (software:4, datascience:4, hardware:4, general_tech:4, company:8)
 * - Rotates 3 per run: full domain coverage every ~5.3 runs (~1.3 hours)
 * - Total: 96 runs × 3 queries = 288 requests/day (90% of 320 cap, 86% of 333 plan limit)
 */
...
// Rate limit: 320 requests/day (safety margin below 333/day RapidAPI plan limit)
// Per-run quota: floor(320 / 96) = 3 queries per 15-min run (288/day baseline).
// The DAILY_REQUEST_CAP is the hard ceiling — if run frequency increases or extra queries
// are added, the cap in fetchFromJSearch() enforces the limit.
const DAILY_REQUEST_CAP = 320;
const RUNS_PER_DAY = 96; // 24h × 4 runs/h (every 15 min)
const QUERIES_PER_RUN = Math.floor(DAILY_REQUEST_CAP / RUNS_PER_DAY); // = 3
...
// One set per consumer repo domain. Biased toward internships (INTERN-2a).
// Tag-engine splits intern vs entry-level downstream.
const QUERY_SETS = {
  // Software Engineering (for New-Grad-Software-Engineering-Jobs-2026 + Internships)
  software: [
    'software engineer intern',
    'software engineering internship 2026',  // replaces 'new grad software engineer'
    'junior software engineer',              // keep — entry-level coverage
    'associate software engineer'            // keep — entry-level coverage
  ],
  // Data Science (for New-Grad-Data-Science-Jobs-2026 + Internships)
  datascience: [
    'data science intern',
    'data science intern summer',            // replaces dead 'data science internship summer 2026' (JSEARCH-DEAD-QUERY)
    'data analyst entry level',              // keep — many are intern-age
    'machine learning intern'               // replaces 'machine learning engineer entry level'
  ],
  // Hardware Engineering (for New-Grad-Hardware-Engineering-Jobs-2026)
  // Dropped 'entry level hardware engineer' (generic, low intern yield) — replaced with internship query.
  // Dropped 'electrical engineer entry level new graduate' — too verbose, low yield.
  hardware: [
    'hardware engineer intern',
    'electrical engineering internship',    // replaces 'entry level hardware engineer'
    'embedded systems engineer entry level', // keep — thin supply, need coverage
    'RF engineer entry level'              // keep (JS-5 addition)
  ],
  // General Tech — broad intern catch-all
  // INTERN-2a: replaced 3 entry-level-only queries with intern/internship queries.
  general_tech: [
    'computer science intern',
    'AI ML internship 2026',               // replaces 'entry level AI engineer'
    'systems engineering internship',      // replaces 'systems engineer entry level'
    'finance internship technology'        // replaces 'associate software developer'
  ],
  // Company-specific — targets unfetchable companies (iCIMS, Oracle HCM, custom portals)
  // These companies have no ATS API access; JSearch catches what their career portals post.
  // SUP-JSEARCH-1 (2026-04-28): 8 queries for 7 missing top-20 tech companies.
  company: [
    ...
    'AMD hardware engineer',
    'Shopify software engineer intern'
  ]
};

// Flat list — all queries across all domains
// 24 total: rotates 3 per run (every 15 min) = full cycle every 8 runs (~2 hours)
const ALL_QUERIES = [
  ...QUERY_SETS.software,
  ...QUERY_SETS.datascience,
  ...QUERY_SETS.hardware,
  ...QUERY_SETS.general_tech,
  ...QUERY_SETS.company
];
...
function selectQueriesForRun(runIndex) {
  // 96 runs/day (every 15 min), 24 queries total, 3 per run
...
async function fetchFromJSearch() {
  ...
  try {
    ...
    for (let i = 0; i < queries.length; i++) {
      ...
      const extraParams = isDS ? dsExtraParams : defaultExtraParams;
