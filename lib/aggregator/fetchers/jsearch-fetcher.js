 ...
 *
 * Features:
 * - Two queries per run (2 queries × 96 runs/day = 192 requests/day, 57% of 333 quota)
 ...
 * - Rotates 2 per run: full domain coverage every ~8.5 runs (~2.1 hours)
 * - Total: 96 runs × 2 queries = 192 requests/day (57% of 333 quota)
 */
...
const QUERIES_PER_RUN = 2; // 2 queries per run × 96 runs/day = 192 requests/day (57% of 333 quota)
...
// 17 total: rotates 2 per run (every 15 min) = full cycle every ~8.5 runs (~2.1 hours)
const ALL_QUERIES = [
  ...QUERY_SETS.software,
...
function selectQueriesForRun(runIndex) {
  // 96 runs/day (every 15 min), 17 queries total, 2 per run
  // Full cycle every 8.5 runs (~2.1 hours) — all domains covered ~11x per day
...
async function fetchFromJSearch() {
  ...
  if (usage.remaining < MAX_REQUESTS_PER_DAY * 0.2) {
    console.warn(`⚠️ JSearch quota low: ${usage.remaining} requests remaining (${Math.round(usage.remaining / MAX_REQUESTS_PER_DAY * 100)}% left)`);
  }
