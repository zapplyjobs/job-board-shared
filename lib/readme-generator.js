...
| *No current openings* | *Check back tomorrow* | *-* | *-* | *-* | *-* |`;
    ...
    Object.entries(jobCategories).forEach(([categoryKey, categoryData]) => {
      ...
      // Group by (title, employer) — show individual rows per location (Simplify pattern).
      // Up to 8 locations shown per role, then "+ N more" summary row.
      // Per-company cap counts role-groups (not individual rows).
      const PER_ROLE_LOCATION_CAP = 8;
      ...
      for (const job of categoryJobs) {
        ...
        if (!roleGroups.has(key)) {
          roleGroups.set(key, [job]);
        } else {
          roleGroups.get(key).push(job);
        }
      }
      // Per-company cap: count role-groups, not individual rows
      const companyRoleCount = {};
      const perCompanyGroups = new Map();
      for (const [key, jobs] of roleGroups) {
        const co = (jobs[0].employer_name || '').toLowerCase();
        companyRoleCount[co] = (companyRoleCount[co] || 0) + 1;
        if (companyRoleCount[co] <= PER_COMPANY_CAP) {
          perCompanyGroups.set(key, jobs);
        }
      }
      // Re-expand with company cap applied
      const displayJobs = [];
      for (const [key, jobs] of perCompanyGroups) {
        const shown = jobs.slice(0, PER_ROLE_LOCATION_CAP);
        shown.forEach((job, i) => { job._isContinuation = i > 0; });
        displayJobs.push(...shown);
        if (jobs.length > PER_ROLE_LOCATION_CAP) {
          const summary = { ...jobs[0] };
          summary._isLocationSummary = true;
          summary._extraLocations = jobs.length - PER_ROLE_LOCATION_CAP;
          displayJobs.push(summary);
        }
        if (displayJobs.length >= TOTAL_CAP + PER_ROLE_LOCATION_CAP) break;
      }
      const finalJobs = displayJobs.slice(0, TOTAL_CAP);
      ...
      finalJobs.forEach((job) => {
        ...
        // Summary row for locations beyond cap
        if (job._isLocationSummary) {
          output += `| | | *+ ${job._extraLocations} locations* | | | |\n`;
          return;
        }

        // Continuation row: same role, different location — group with ↳
        const isContinuation = job._isContinuation;
        const companyCell = isContinuation ? '↳' : `${emoji} **${companyName}**`;
        const roleCell = isContinuation ? '' : role;
        ...
        // WD internal addresses: "STATE-UPPERCASE CITY-CODE" or "STATE-UPPERCASE CITY-CODE--ADDRESS"
        // e.g. "PA-CHAMBERSBURG-5808-CUST--1-Overcash-Ave" → "Chambersburg, PA"
        // Match: STATE code, then uppercase city words, then dash+alphanumeric code or end
        const wdAddr = locationRaw.match(/^([A-Z]{2})-((?:[A-Z]+(?: [A-Z]+)*?))(?:-[A-Z0-9]{1,}.*|$)/);
        if (wdAddr && wdAddr[2].trim().length > 1) {
          const cityWords = wdAddr[2].trim().split(/\s+/);
          locationRaw = cityWords.map(w => w.charAt(0) + w.slice(1).toLowerCase()).join(' ') + ', ' + wdAddr[1];
        }
        ...
        const visa = isContinuation ? '' : (sponsorsVisa ? "✅ Sponsoring Company" : (visaQuestion ? "🟢 Sponsoring Job" : (possibleSponsor ? "🟡 Likely Sponsor" : "")));

        output += `| ${companyCell} | ${roleCell} | ${location} | ${posted} | ${visa} | [<img src="images/apply.png" width="120" alt="Apply">](${applyLink}) |\n`;
      });
