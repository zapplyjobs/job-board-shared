...
function createReadmeGenerator(config, jobCategories, repoRoot) {
  ...
  function generateJobTable(jobs) {
    ...
    if (jobs.length === 0) {
      return `| Company | Role | Skills | Visa | Location | Posted | Apply |
|---------|------|--------|------|----------|--------|-------|
| *No current openings* | *Check back tomorrow* | *-* | *-* | *-* | *-* | *-* |`;
    ...
    Object.entries(jobCategories).forEach(([categoryKey, categoryData]) => {
      ...
      output += `| Company | Role | Skills | Visa | Location | Posted | **Apply** |\n`;
      output += `|---------|------|--------|------|----------|--------|----------|\n`;
      ...
      displayJobs.forEach((job) => {
        ...
        // Enrichment columns — show skills and visa when available
        const enr = job.enrichment || {};
        const skills = (enr.required_skills || []).slice(0, 3).join(', ').replace(/\|/g, '');
        const visa = (enr.sponsors_visa === true || enr.visa_question_present === true)
          ? '✅ Sponsor'
          : (enr.sponsors_visa === false ? '' : '');

        output += `| ${emoji} **${companyName}** | ${role} | ${skills} | ${visa} | ${location} | ${posted} | [<img src="images/apply.png" width="75" alt="Apply">](${applyLink}) |\n`;
      });
