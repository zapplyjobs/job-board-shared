...
| *No current openings* | *Check back tomorrow* | *-* | *-* | *-* |`;
    ...
    Object.entries(jobCategories).forEach(([categoryKey, categoryData]) => {
      ...
      output += `<details>\n`;
      output += `<summary><h3>${categoryData.emoji} <strong>${categoryData.title}</strong> (${categoryJobs.length} positions)</h3></summary>\n\n`;
      ...
      categoryJobs.forEach((job) => {
        ...
        output += `| ${emoji} **${companyName}** | ${role}${statusIndicator} | ${location} | ${posted} | [<img src="images/apply.png" width="75" alt="Apply">](${applyLink}) |\n`;
      });
...
> ⏱ Last updated: ${new Date().toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit', timeZone: 'America/Chicago', timeZoneName: 'short' })}
...
${config.repoPrefix !== 'ngj' ? `<p align="center">
