...
`;
  ...
  async function generateReadme(currentJobs, archivedJobs = [], internshipData = null, stats = null) {
    ...
    // Get top category for badge (skip "Other" default/catch-all categories)
    const filteredCategories = Object.entries(currentStats.byCategory).filter(([category]) => !category.startsWith('Other'));
    const topCategoryEntry = filteredCategories.sort((a, b) => b[1] - a[1])[0];
...
![${topCategory}](https://img.shields.io/badge/${topCategoryBadge}-${topCategoryCount}-purple?style=flat&logo=tag)
