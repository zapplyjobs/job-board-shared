...
| *No current openings* | *Check back tomorrow* | *-* | *-* | *-* |`;
    ...
    Object.entries(jobCategories).forEach(([categoryKey, categoryData]) => {
      ...
      finalJobs.forEach((job) => {
        ...
        // Work-type labels masquerading as locations: "In-Office", "Hybrid", "Remote"
        // These have no city/state — the location field is a work arrangement, not a place.
        const WORK_TYPE_RE = /^(In-Office|Hybrid|On-Site|Onsite|Remote|Various|Multiple)$/i;
        if (WORK_TYPE_RE.test(locationRaw.trim())) {
          locationRaw = locationRaw.trim().toLowerCase().startsWith('remote') ? 'Remote' : formatLocation(job.job_city, job.job_state);
        }
