...
function createReadmeGenerator(config, jobCategories, repoRoot) {
  ...
  function generateJobTable(jobs) {
    ...
    if (jobs.length === 0) {
      return `| Company | Role | Visa | Location | Posted | Apply |
|---------|------|------|----------|--------|-------|
| *No current openings* | *Check back tomorrow* | *-* | *-* | *-* | *-* |`;
    ...
    Object.entries(jobCategories).forEach(([categoryKey, categoryData]) => {
      ...
      output += `| Company | Role | Visa | Location | Posted | **Apply** |\n`;
      output += `|---------|------|------|----------|--------|----------|\n`;
      ...
      displayJobs.forEach((job) => {
        ...
        // Visa sponsorship from enrichment — high-signal for international students
        // Check all three signals: LCA H-1B filing history (possible_sponsor), ATS form
        // detection (visa_question_present), and explicit statement (sponsors_visa).
        // Session B S268 finding: only checking two of three left 76% of jobs without badge
        // despite 92% enrichment coverage.
        const enr = job.enrichment || {};
        const visa = (enr.possible_sponsor === true || enr.sponsors_visa === true || enr.visa_question_present === true)
          ? '✅ Sponsor'
          : '';

        output += `| ${emoji} **${companyName}** | ${role} | ${visa} | ${location} | ${posted} | [<img src="images/apply.png" width="75" alt="Apply">](${applyLink}) |\n`;
      });
