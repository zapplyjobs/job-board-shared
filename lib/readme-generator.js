...
| *No current openings* | *Check back tomorrow* | *-* | *-* | *-* |`;
    ...
    Object.entries(jobCategories).forEach(([categoryKey, categoryData]) => {
      ...
      output += `| Company | Role | Location | Skills | Posted | **Apply** |\n`;
      output += `|---------|------|----------|--------|--------|----------|\n`;
      ...
      displayJobs.forEach((job) => {
        ...
        // Enrichment data (from mergeEnrichmentData in aggregator-consumer)
        const enr = job.enrichment || {};
        const skills = (enr.required_skills || []).slice(0, 3).join(', ');
        const skillsCell = skills || '-';
        // Visa indicator: 🛂 = sponsors, blank = unknown (don't show ❌ — absence ≠ refusal)
        const visa = enr.sponsors_visa === true ? ' 🛂' : '';

        output += `| ${emoji} **${companyName}** | ${role}${visa} | ${location} | ${skillsCell} | ${posted} | [<img src="images/apply.png" width="75" alt="Apply">](${applyLink}) |\n`;
      });
