...
function createReadmeGenerator(config, jobCategories, repoRoot) {
  ...
  // Normalize title for grouping — strips ATS-appended location suffixes.
  // Greenhouse appends " - City, ST, USA" to titles (e.g., "Software Engineer, iOS Core Product - Washington, DC, USA").
  // Without normalization, each location variant gets its own row.
  // Only strips trailing patterns that match known ATS location formats.
  function normalizeTitleForGrouping(title) {
    if (!title) return '';
    // Strip " - City, ST, USA" or " - City, ST" (GH pattern)
    // Match: dash + space + City (letters/spaces/dots/apostrophes) + comma + 2-letter state + optional ", USA"
    // Dots needed for "St. Louis", "St. Petersburg", "Washington, D.C."
    let normalized = title.replace(/\s*[-–—]\s*[A-Z][a-zA-Z\s.'-]+,\s*[A-Z]{2}(?:,\s*USA)?\s*$/, '');
    // Strip trailing " - Remote" / " - Hybrid" / " - Onsite" (employment type suffix)
    normalized = normalized.replace(/\s*[-–—]\s*(?:Remote|Hybrid|Onsite)\s*$/i, '');
    return normalized;
  }
...
| *No current openings* | *Check back tomorrow* | *-* | *-* | *-* | *-* |`;
    ...
    Object.entries(jobCategories).forEach(([categoryKey, categoryData]) => {
      ...
      // Group by (title, employer) — same role at multiple locations becomes one row.
      // Each group keeps the newest job as the display row + count of locations.
      // Sort is newest-first so first occurrence has the most recent posted date.
      const roleGroups = new Map();
      for (const job of categoryJobs) {
        const normalizedTitle = normalizeTitleForGrouping(job.job_title || '');
        const key = `${normalizedTitle.toLowerCase()}|${(job.employer_name || '').toLowerCase()}`;
        if (!roleGroups.has(key)) {
          roleGroups.set(key, { job, locationCount: 1 });
        } else {
          roleGroups.get(key).locationCount++;
        }
      }
      const dedupedJobs = Array.from(roleGroups.values()).map(g => {
        g.job._locationCount = g.locationCount;
        return g.job;
      });
      ...
      displayJobs.forEach((job) => {
        ...
        const locationBase = locationTrunc.replace(/\|/g, '').trim();
        const locationCount = job._locationCount || 1;
        const location = locationCount > 1 ? `${locationBase} + ${locationCount - 1} more` : locationBase;
        ...
        const visaSignal = job.enrichment?.sponsors_visa || job.enrichment?.visa_question_present;
        const lcaSignal = job.enrichment?.possible_sponsor;
        const visa = visaSignal ? "Yes" : (lcaSignal ? "LCA" : "");
