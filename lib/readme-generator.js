...
| *No current openings* | *Check back tomorrow* | *-* | *-* | *-* | *-* |`;
    ...
    Object.entries(jobCategories).forEach(([categoryKey, categoryData]) => {
      ...
      displayJobs.forEach((job) => {
        ...
        // Visa sponsorship — 3-tier signal (S274):
        // ✅ Sponsors Visa = job text or ATS form explicitly mentions visa (hard evidence)
        // 🏢 Sponsor Employer = LCA database match only (company-level evidence, not this job)
        // blank = no evidence found
        const enr = job.enrichment || {};
        const hasHard = enr.sponsors_visa === true || enr.visa_question_present === true;
        const hasLca = enr.possible_sponsor === true;
        const visa = hasHard ? '✅ Sponsors Visa' : (hasLca ? '🏢 Sponsor Employer' : '');
