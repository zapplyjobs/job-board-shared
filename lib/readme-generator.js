...
| *No current openings* | *Check back tomorrow* | *-* | *-* | *-* | *-* |`;
    ...
    Object.entries(jobCategories).forEach(([categoryKey, categoryData]) => {
      ...
      categoryJobs.forEach((job) => {
        ...
        const location = locationRaw.length > 20 ? locationRaw.substring(0, 17) + "..." : locationRaw;
...
`;
  ...
  async function generateReadme(currentJobs, archivedJobs = [], internshipData = null, stats = null) {
    ...
    // Config strings used directly (template-renderer.js removed in N-6)
    const renderedConfig = {
      descriptionLine1: config.descriptionLine1,
      descriptionLine2: config.descriptionLine2
    };
