...
// Config
// ---------------------------------------------------------------------------
const ENRICHER_VERSION = 39;   // ENR-LCA-1 + ENR-VISA-1: remove possible_sponsor, add visa_no_signal_reason
...
function classifyVisaGap(companyName, sponsorsVisa, visaQuestionPresent) {
// Only classify records with truly zero visa signal
if (sponsorsVisa !== null || visaQuestionPresent !== null) return null;
...
const visaNoSignalReason = classifyVisaGap(job.company_name, sponsorsVisa, visaQuestionPresent);
...
if (obj.visa_no_signal_reason !== null) statsBySource[src].visa_explained_gaps++;
// any_visa_signal: text + form signal (ENR-LCA-1 removed possible_sponsor from new records,
// but old records in JSONL still have it — count backward-compat)
const hasSponsorOrForm = obj.sponsors_visa !== null || obj.visa_question_present !== null;
const hasLegacyLca = obj.possible_sponsor !== null;
if (hasSponsorOrForm || hasLegacyLca) statsBySource[src].any_visa_signal++;
