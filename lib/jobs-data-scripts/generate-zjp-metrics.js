...
async function main() {
  ...
  if (metadata?.stage_timings) {
    ...
    // Derive execution time from stage_timings when Jobs API didn't provide it
    ...
    if (stageTotalMs > 0) {
      ...
      if (!pipeline.aggregator_execution_minutes || pipeline.aggregator_execution_minutes === null) {
        metrics.pipeline.aggregator_execution_minutes = stageExecMin;
      }
      // Derive queue from wall - execution when queue is missing
      if ((!pipeline.aggregator_queue_minutes || pipeline.aggregator_queue_minutes === null) && pipeline.aggregator_runtime_minutes) {
        const derivedQueue = Math.max(0, Math.round((pipeline.aggregator_runtime_minutes - stageExecMin) * 10) / 10);
        metrics.pipeline.aggregator_queue_minutes = derivedQueue;
      }
    }
  }
