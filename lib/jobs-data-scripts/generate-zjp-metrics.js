...
async function main() {
  ...
  if (metadata?.stage_timings) {
    ...
    if (stageTotalMs > 0) {
      ...
      // stage_timings is the most accurate pipeline execution measurement (internal instrumentation)
      // Always use it over Jobs API timing which includes GH Actions overhead
      metrics.pipeline.aggregator_execution_minutes = stageExecMin;
      // Derive queue from wall-clock minus execution
      if (pipeline.aggregator_runtime_minutes) {
