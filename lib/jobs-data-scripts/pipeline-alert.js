...
async function runChecks() {
  ...
  if (!fs.existsSync(metadataPath)) {
    ...
    // Check 13 (INF-SCALE-2): Runtime threshold — alert on execution time, not wall time.
    // Previously measured wall time (execution + queue), causing false alerts from queue overlap.
    // Now uses stage_timings from jobs-metadata.json for true execution time.
    // Fallback: wall time from API when stage_timings unavailable.
    const RUNTIME_EXECUTION_THRESHOLD = 20; // minutes (execution only, excluding queue)
    try {
      // Try stage_timings from current metadata first (most accurate)
      const stageTimings = metadata?.stage_timings;
      let currentExecutionMin = null;
      if (stageTimings) {
        const stageTotalMs = Object.values(stageTimings).reduce((a, b) => a + (typeof b === 'number' ? b : 0), 0);
        if (stageTotalMs > 0) currentExecutionMin = stageTotalMs / 60000;
      }

      if (currentExecutionMin !== null && currentExecutionMin > RUNTIME_EXECUTION_THRESHOLD) {
        // Use stage_timings (execution-only) — already have precise data
        const queueMin = currentExecutionMin < (metadata?.wall_time_min || 999)
          ? Math.max(0, ((metadata?.wall_time_min || currentExecutionMin) - currentExecutionMin)).toFixed(1)
          : 'unknown';
        failures.push(
          `**Aggregator execution time high**: ${currentExecutionMin.toFixed(1)} min (threshold: ${RUNTIME_EXECUTION_THRESHOLD} min). Queue: ${queueMin} min. At 25 min execution, 15-min cadence breaks.`
        );
      } else {
        // Fallback: check API for recent run times (less precise — includes queue)
        ...
        if (recentRuns.status === 200 && recentRuns.body?.workflow_runs) {
          ...
          if (successfulRuns.length >= 2) {
            ...
            // Use higher threshold for wall-time fallback (queue inflates it)
            if (avgRuntime > 30) {
              failures.push(
                `**Aggregator runtime high** (wall time, incl. queue): avg ${avgRuntime.toFixed(1)} min over ${successfulRuns.length} runs (max ${maxRuntime.toFixed(1)} min). Execution data unavailable — check stage_timings in metadata.`
              );
            }
          }
        }
  ...
  // Check 20 (INF-OBSERV-10): Carry-forward staleness — repos with stale data >24h
  // collect-metrics.js writes stale_warnings to metrics/latest.json when carry-forward
  // entries persist beyond 24h. Surfaces here so operators know metrics are unreliable.
  try {
    if (fs.existsSync(METRICS_LATEST)) {
      const metricsLatest = JSON.parse(fs.readFileSync(METRICS_LATEST, 'utf8'));
      const staleWarnings = metricsLatest.stale_warnings || [];
      if (staleWarnings.length > 0) {
        const shown = staleWarnings.slice(0, 10);
        const suffix = staleWarnings.length > 10 ? ` (+${staleWarnings.length - 10} more)` : '';
        failures.push(
          `**Metrics carry-forward stale** (${staleWarnings.length} repo(s) >24h): ` +
    ...
    console.error('Error checking stale carry-forward:', err.message);
  }
